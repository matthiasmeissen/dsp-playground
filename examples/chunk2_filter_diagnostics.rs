...
fn main() -> anyhow::Result<()> {
    ...
    let mut diag = AudioDiagnostics::new(sample_rate);
    ...
    let stream = device.build_output_stream(
        ...
        move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
            for frame in data.chunks_mut(channels) {
                ...
                svf_filter.set_params(cutoff, 1.0);
                ladder_filter.set_cutoff(cutoff);
                ladder_filter.set_resonance(0.3);
    ...
    println!("Filter diagnostics. Switch: l=ladder, s=svf. Cutoff: c <hz>. Quit: q");
